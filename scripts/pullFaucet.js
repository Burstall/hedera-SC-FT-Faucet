...
const main = async () => {
	...
	const ethFlag = getArgFlag('eth');
	const htsFlag = getArgFlag('hts');
	let method = '';
	...
	if (ethFlag) {
		console.log('\n-Using IERC20 method');
		method = 'pullFaucetETH';
	}
	else if (htsFlag) {
		console.log('\n-Using HTS method');
		method = 'pullFaucetHTS';
	}
	else {
		console.log('Specify -eth or -hts methods');
		return;
	}
	...
	const pull = await pullFaucet(method);
	console.log('Tx:', pull);
};
...
async function pullFaucet(fcnName) {
